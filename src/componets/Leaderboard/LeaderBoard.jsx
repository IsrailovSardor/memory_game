...
const LeaderBoard = () => {
    const parseScore = JSON.parse(localStorage.getItem("score")) ? JSON.parse(localStorage.getItem("score")) : [];
            ...
            <div className="leaderboard__container">
                ...
                <div className="leaderboard__info">
                    <div className="info__header">
                        ...
                        <Link to="/" data-testid='main-link' className="back">
                            Home
                        </Link>
                    </div>
                    <div className="dropdown_block">
                        <div className="dropdown">
                            <div className="dropdown-header" onClick={toggleDropdown}>
                                {selectedItem ? items.find((item) => item.id == selectedItem).label : "Сортировка по"}
                        ...
                        <div className="dropdown">
                            <div className="dropdown-header" onClick={toggleDropdowns}>
                                {selectedItems ? itemss.find((item) => item.id == selectedItems).label : "Выберите режим"}
                                <ArrowDropUpIcon className={`icon ${isOpens && "open"}`} />
                            </div>
                            <div className={`dropdown-body ${isOpens && "open"}`}>
                                {itemss.map((item) => (
                                    <div key={item.id} className="dropdown-item" onClick={(e) => handleItemClicks(e.target.id)} id={item.id}>
                                        <span className={`dropdown-item-dot ${item.id == selectedItems && "selected"}`}></span>
                                        {item.label}
                                    </div>
                                ))}
                            </div>
                            ...
                            <>
                                {parseScore
                                    ...
                                    .map((user, index) => {
                                        ...
                                        return (
                                            <div className={`info__user ${userName === user.name ? "blue_mode" : "white_mode"}`} key={index}>
                                                ...
                                                <div className="user__name">
                                                    ...
                                                    <div className="user__name-two">
                                                        {activSort === 2 ? (
                                                            ...
                                                            <>
                                                                {activSort == 1 ? <p>score: {user.score}</p> : null}
                                                                {activSort == 0 ? (
                                                                    <p>
                                                                        time: {m.toString().padStart(2, "0")}:{s.toString().padStart(2, "0")}:
                                                                        {ms.toString().padStart(3, "0")}
                                                                ...
                                                                {activSort == 2 ? <p>moves: {user.moves}</p> : null}
                                                            </>
                                                        )}
                                                    </div>
                                                </div>
